...
function viewBudget() {
    ...
    .then(function(data){ 
        if(data.success){
            var budgetSpentView = data.budget_spent;
            document.getElementById("budget_spent").innerHTML = budgetSpentView;
        }
        else {
            throw data.message;
        }
    ...
    .catch(function(error){
        window.alert("errore nella visualizzazione del budget speso");    
    })
}